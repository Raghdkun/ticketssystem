...
const HoldButton: React.FC<HoldButtonProps> = ({
    ...
    const drive = (
        ...
        const step = (now: number) => {
            const t =
                duration > 0 ? Math.min(1, (now - m.start) / duration) : 1;
        ...
        >
            ...
            >
                <span className="hb-fill absolute inset-0 grid place-items-center [color:var(--hb-fill-text)] [background:var(--hb-fill)]">
                    <span className="hb-label grid place-items-center">
                        {labels}
                    </span>
                </span>
            ...
            >
                Press and hold for {Math.round(holdTime / 100) / 10} seconds to
                confirm
            </span>
        </button>
    );
